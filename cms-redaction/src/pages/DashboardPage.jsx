...
export default function DashboardPage() {
  ...
  return (
    <div className="p-8 max-w-6xl">
      ...
      {/* Disposition volontairement différente du Quotidien : fil dense,
          horodatage en tête, titre en serif — dans l'esprit d'un fil
          d'actualité en direct (cf. inspiration lemonde.fr / nytimes.com)
          plutôt que la liste éditoriale classique du journal imprimé. */}
      {estInfoDirect ? (
        <div className="card mt-8">
          <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #232B45' }}>
            <span className="flex items-center gap-1.5 font-mono text-[10.5px] font-bold uppercase tracking-widest" style={{ color: '#22D3EE' }}>
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: '#22D3EE' }} /> Fil de la rédaction
            </span>
            <Link to="/articles" className="text-xs font-semibold hover:underline" style={{ color: '#22D3EE' }}>Voir tout →</Link>
          </div>
          {!loading && recents.length === 0 && (
            <p className="p-6 text-sm text-center" style={{ color: '#6B7694' }}>Aucun article pour le moment. Créez le premier !</p>
          )}
          {recents.map((a, i) => (
            <Link
              key={a.id}
            ...
            >
              ...
              <span className="text-[10.5px] font-mono shrink-0 hidden sm:inline" style={{ color: '#6B7694' }}>{a.auteur?.prenom} {a.auteur?.nom}</span>
