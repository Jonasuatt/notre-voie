...
export default async function RubriquePage({ params, searchParams }) {
  ...
  const [{ articles, total }, campagnes, editions] = await Promise.all([
    getArticles({ rubrique: params.slug, date: date || undefined, pageSize: 30, portail: 'QUOTIDIEN' }),
    ...
    <section className="max-w-[1180px] mx-auto px-4 sm:px-8 py-10">
      ...
      <form method="GET" className="flex flex-wrap items-end gap-3 mb-6 -mt-2">
        <div>
          ...
          <input type="date" name="date" defaultValue={date} className="border border-line rounded-md px-3 py-1.5 text-[13px]" />
        </div>
        <button type="submit" className="text-[12.5px] font-bold text-white bg-navy rounded-full px-4 py-1.5">Filtrer</button>
        {date && (
