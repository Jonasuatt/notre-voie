...
export default function Carrousel({ titre, articles, basePath = '' }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between pb-3 mb-6 border-b" style={{ borderColor: '#2E6D9E' }}>
        ...
        <div className="flex gap-2">
          <button
            ...
            style={{ borderColor: '#2E6D9E', color: '#EAF3FB' }}
          >
            ‹
          </button>
          <button
      ...
      <div ref={pisteRef} className="flex gap-5 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-fine">
        {articles.map((a) => (
          ...
          >
            ...
            <h4 className="font-serif text-[15px] leading-snug mt-1 line-clamp-3 group-hover:text-[#4FB3F0] transition-colors">{a.titre}</h4>
