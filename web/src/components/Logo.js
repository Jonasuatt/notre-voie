...
export default function LogoPill({ small = false }) {
  const size = small ? 'h-[26px] text-[11px] px-[9px]' : 'h-9 text-[15px] px-[13px]';
  return (
    <Link href="/" className="inline-flex items-center rounded-full overflow-hidden shadow-sm font-serif font-extrabold">
