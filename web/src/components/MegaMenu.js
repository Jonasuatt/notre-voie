      ...
      {piliers.map((pilier) => (
        <div key={pilier.cle} onMouseEnter={() => ouvrir(pilier.cle)} onMouseLeave={fermerDifferee}>
          <button
            ...
            className={`hover:text-[#4FB3F0] transition-colors ${ouvert === pilier.cle ? 'text-[#4FB3F0]' : ''}`}
          ...
          {ouvert === pilier.cle && (
            <div
              className="fixed left-0 right-0 top-[72px] bg-[#0E3C63] border-t border-b border-[#2E6D9E] shadow-2xl z-40"
            ...
            >
              ...
              >
                <div>
                  <span className="block font-mono text-[10.5px] uppercase tracking-widest text-[#A9C6DD] mb-3">Rubriques</span>
                    ...
                    {pilier.liensRubriques.map((r) => (
                      <li key={r.href}>
                        <Link
                          ...
                          className="inline-block text-[14px] font-normal text-[#EAF3FB] hover:text-[#4FB3F0] hover:translate-x-1 transition-all"
                        >
                          {r.label}
                        </Link>
                        {r.sousRubriques?.length > 0 && (
                          <ul className="mt-1.5 ml-3 space-y-1 border-l border-[#2E6D9E] pl-3">
                            {r.sousRubriques.map((sr) => (
                              <li key={sr.href}>
                                <Link
                                  href={`${basePath}${sr.href}`}
                                  onClick={() => setOuvert(null)}
                                  className="inline-block text-[12.5px] text-[#A9C6DD] hover:text-[#4FB3F0] hover:translate-x-1 transition-all"
                                >
                                  {sr.label}
                                </Link>
                              </li>
                    ...
                    {pilier.liensSupplementaires?.map((l) => (
                      <li key={l.href}>
                        <Link
                          ...
                          onClick={() => setOuvert(null)}
                          className="inline-flex items-center gap-2 text-[14px] font-normal text-[#EAF3FB] hover:text-[#4FB3F0] hover:translate-x-1 transition-all"
                        >
                ...
                {pilier.aLaUne.length > 0 && (
                  <div>
                    <span className="block font-mono text-[10.5px] uppercase tracking-widest text-[#A9C6DD] mb-3">En ce moment</span>
                    <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                      {pilier.aLaUne.map((a) => (
                        ...
                        >
                          {a.imageUneUrl && (
                            <div className="relative w-[76px] h-[76px] shrink-0 rounded-[6px] overflow-hidden bg-[#17527F]">
                              <Image src={a.imageUneUrl} alt="" fill sizes="76px" className="object-cover" />
                            </div>
                          )}
                          <div className="min-w-0">
                            <span className="font-mono text-[10px] uppercase tracking-wide" style={{ color: a.rubrique?.couleur || '#4FB3F0' }}>
                              {a.rubrique?.nom}
                            </span>
                            <p className="text-[13px] leading-snug mt-1 font-serif line-clamp-2 group-hover:text-[#4FB3F0] transition-colors">
                              {a.titre}
                            </p>
                          </div>
                        </Link>
...
function BlocSecours({ secours, basePath, onNaviguer }) {
  const conteneur = 'rounded-[10px] border border-[#2E6D9E] bg-[#12456F] p-4 flex flex-col h-full';

  if (secours.type === 'kiosque' && secours.edition) {
    const { edition } = secours;
    return (
      <div className={conteneur}>
        <span className="font-mono text-[10.5px] uppercase tracking-widest text-[#A9C6DD] mb-3">Kiosque numérique</span>
        <div className="relative aspect-[3/4] rounded-[6px] overflow-hidden mb-3 bg-[#0B3358]">
          <Image src={edition.couvertureUrl} alt={`Une n°${edition.numero}`} fill sizes="200px" className="object-cover" />
        </div>
        <p className="text-[12px] text-[#A9C6DD] mb-3">N°{edition.numero} — édition du jour</p>
        <Link
          href={`${basePath}/kiosque`}
          onClick={onNaviguer}
          className="mt-auto text-center text-[12.5px] font-bold bg-[#4FB3F0] text-[#062134] rounded-full py-2 hover:brightness-95 transition"
        >
  ...
  if (secours.type === 'audio' && secours.article) {
    ...
    return (
      <div className={conteneur}>
        <span className="font-mono text-[10.5px] uppercase tracking-widest text-[#A9C6DD] mb-3">🎧 Dernier podcast</span>
        ...
        <Link
          ...
          onClick={onNaviguer}
          className="mt-2 text-center text-[12px] text-[#A9C6DD] hover:text-[#4FB3F0] transition-colors"
        >
          Tous les épisodes →
        </Link>
      </div>
  ...
  return (
    <div className={conteneur}>
      <span className="font-mono text-[10.5px] uppercase tracking-widest text-[#A9C6DD] mb-3">Notre Voie</span>
